...
WEB_SERVERS="auto"  # web servers are ASG-managed with dynamic IPs;
                    # "auto" discovers them by tag via the AWS CLI
SSH_KEY="sshkeys_generated/<project>-<environment>-web"  # generated by Terraform
JUMP_HOST="bastion-host-ip"  # Optional bastion host
```

Alternatively, skip the helper's SSH plumbing entirely with the generated
ssh config: `ssh -F sshcfg ubuntu@<web-ip> 'sudo vhost list'`.
