...
3. Your admin IP ranges for bastion SSH access (`bastion_allowed_cidrs` is required; `0.0.0.0/0` is rejected)
4. Optional: an ACM certificate in the deployment region for HTTPS
5. Optional: Packer, if you want to pre-bake the web AMI (see packer/README.md)

SSH keys are generated by Terraform — one key pair per role (bastion, web,
database, monitoring) written to `sshkeys_generated/` (gitignored), plus a
ready-made `./sshcfg`. No pre-existing AWS key pair is needed.
...
#### Bastion Host Access
The infrastructure includes a bastion host for secure access to private instances. Terraform generates per-role SSH keys (`sshkeys_generated/`) and a ready-made ssh config (`./sshcfg`):

1. **Connect to bastion host**:
```bash
ssh -F sshcfg bastion
# add the internal keys to your agent first if you'll hop onward from the
# bastion (its tools use agent forwarding; keys are never stored on it):
...
3. **Direct SSH from your local machine** (ProxyJump is wired into `sshcfg`):
```bash
ssh -F sshcfg db1                          # database master 1
ssh -F sshcfg db2                          # database master 2
ssh -F sshcfg monitoring                   # monitoring server
ssh -F sshcfg ubuntu@[web-private-ip]      # any web server (dynamic IPs)
```
...
```bash
# SSH to bastion host first (generated config + per-role keys)
ssh -F sshcfg bastion
...
WEB_SERVERS="auto"  # auto-discovers via AWS CLI (web IPs are dynamic)
SSH_KEY="sshkeys_generated/[project]-[environment]-web"
```
