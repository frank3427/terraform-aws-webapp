...
# Configure MariaDB for replication (Master 2)
...
mysql -e "
CREATE USER IF NOT EXISTS '${db_replication_user}'@'${master1_ip}' IDENTIFIED BY '$DB_REPL_PASSWORD';
GRANT REPLICATION SLAVE ON *.* TO '${db_replication_user}'@'${master1_ip}';
FLUSH PRIVILEGES;
"
...
# Wait for Master 1 to be ready (simple wait, in production use better coordination)
sleep 120

# Setup replication from Master 1
mysql -e "
CHANGE MASTER TO
...
echo "MariaDB Master 2 setup completed successfully"
